...
3. **テンプレートダッシュボード**
   ...
   - `docs/archive/grafana_pyoco_cute.json` は `Runs in progress` スタット、`Run completions (last 5m)` タイムシリーズ、`Average run duration by flow` テーブルの3枚構成。背景はダークテーマ、色は `#cdb4db` と `#ffafcc/#ffb562` を基調にして「かわいさ」を演出。
4. **Webhook通知 (任意設定)**
