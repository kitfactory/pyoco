...
この章では、workflow の結果を「あとで使える形」で残す方法を扱います。結果が再利用できると、flow は一段と実用的になります。
...
後続 task がその場で読む値なら `outputs`、人や別ツールがあとで見るファイルなら artifact、と考えると整理しやすいです。
