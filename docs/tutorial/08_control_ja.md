# チュートリアル 8: 制御と可観測性 (Control & Observability)

この章では、flow が「本当に動いているもの」になったときに効いてくる話を扱います。どの実行かを識別し、観測し、必要なら安全に止めるための基本です。
...
この ID はログの相関やローカルでのデバッグに利用できます（サーバー機能はアーカイブ済み）。
...
この章では、すぐ試せるように local `callable` で束ねています。再利用する task に育てる場合は、同じ task を plug-in として公開して `use` で束ねる形へ進めます。
